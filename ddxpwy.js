...
function ddwyck() {
  if ($request.url.indexOf("此脚本无需获取ck") > -1) {
    ...
    $.msg($.name, "", `叮咚喂鱼${status}获取headers成功`)
...
function fwmm2(timeout = 0) {
  return new Promise((resolve) => {
    ...
    let url = {
      url: `https://farm.api.ddxq.mobi/api/v2/userguide/orchard/detail?api_version=9.1.0&app_client_id=1&native_version=&uid=${uid}&latitude=${latitude}&longitude=${longitude}&gameId=2&cityCode=0101`,
      headers: headers,
    }
...
function gyjs(timeout = 0) {
  return new Promise((resolve) => {
    ...
    let url = {
      url: `https://farm.api.ddxq.mobi/api/v2/props/feed?api_version=9.1.0&app_client_id=1&native_version=&uid=${uid}&latitude=${latitude}&longitude=${longitude}&propsCode=FEED&seedId=${seedId2}&propsId=${propsId2}`,
      headers: headers,
    }
